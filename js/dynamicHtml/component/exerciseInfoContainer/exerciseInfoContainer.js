...
function createExerciseCategoryClassification(cardInfo) {
	// 카테고리 정보 만드는 함수
...
function createExerciseVideoFrame(youtubeLink) {
	// 비디오 프레임 만드는 함수
	// 비디오 프레임 만들기
...
function createDivForSentences(id, title, sentences, listStyle) {
	...
	for (let i = 0; i < sentences.length; i++) {
		var paragraph = document.createElement("p");
		if (listStyle === "num") paragraph.textContent = i + 1 + ". " + sentences[i];
		else paragraph.textContent = sentences[i];
		div.appendChild(paragraph);
	}
	return div;
}
